...
//!
//! Uses sqlx for async database operations with MySQL backend.
...
#[async_trait]
impl RunRepository for MysqlRepository {
    async fn insert_run(
        &self,
        id: &str,
        job_id: &str,
...
#[async_trait]
impl ArtifactRepository for MysqlRepository {
...
#[async_trait]
impl TriggerRepository for MysqlRepository {
    async fn upsert_trigger(&self, trigger: &TriggerInfo) -> Result<()> {
        sqlx::query(
            "INSERT INTO triggers (name, cron, job_id, enabled) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE cron = VALUES(cron), job_id = VALUES(job_id), enabled = VALUES(enabled)",
        )
        .bind(&trigger.name)
        .bind(&trigger.cron)
        .bind(&trigger.job_id)
...
#[async_trait]
impl WebhookRepository for MysqlRepository {
    async fn upsert_webhook_trigger(&self, webhook: &WebhookTriggerInfo) -> Result<()> {
        ...
        sqlx::query(
            "INSERT INTO webhook_triggers (name, job_id, enabled, secret, source, filter, credential_id) VALUES (?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE job_id = VALUES(job_id), enabled = VALUES(enabled), secret = VALUES(secret), source = VALUES(source), filter = VALUES(filter), credential_id = VALUES(credential_id)",
        )
        .bind(&webhook.name)
        .bind(&webhook.job_id)
...
#[async_trait]
impl VcsCredentialRepository for MysqlRepository {
    async fn upsert_credential(&self, cred: &VcsCredentialInfo) -> Result<()> {
        sqlx::query(
            "INSERT INTO vcs_credentials (id, name, vcs_type, username, credential) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE name = VALUES(name), vcs_type = VALUES(vcs_type), username = VALUES(username), credential = VALUES(credential)",
        )
        .bind(&cred.id)
        .bind(&cred.name)
...
#[async_trait]
impl ApiTokenRepository for MysqlRepository {
    ...
    async fn get_token_by_hash(&self, token_hash: &str) -> Result<Option<ApiTokenInfo>> {
        let row: Option<(i64, String, String, String, String, Option<String>, Option<String>, String)> = sqlx::query_as(
            "SELECT id, name, token_hash, permissions, CAST(created_at AS CHAR), CAST(expires_at AS CHAR), CAST(last_used AS CHAR), created_by FROM api_tokens WHERE token_hash = ?",
        )
        .bind(token_hash)
        .fetch_optional(&self.pool)
        .await
    ...
    async fn list_tokens(&self) -> Result<Vec<ApiTokenInfo>> {
        let rows: Vec<(i64, String, String, String, String, Option<String>, Option<String>, String)> = sqlx::query_as(
            "SELECT id, name, token_hash, permissions, CAST(created_at AS CHAR), CAST(expires_at AS CHAR), CAST(last_used AS CHAR), created_by FROM api_tokens ORDER BY created_at DESC",
        )
        .fetch_all(&self.pool)
        .await
        .map_err(|e| DbError::Query(e.to_string()))?;
    ...
    async fn update_token_last_used(&self, token_id: i64) -> Result<()> {
        sqlx::query("UPDATE api_tokens SET last_used = NOW() WHERE id = ?")
            .bind(token_id)
            .execute(&self.pool)
            .await
            .map_err(|e| DbError::Query(e.to_string()))?;
...
mod tests {
    ...
    async fn create_test_repo() -> MysqlRepository {
        ...
        // Use MYSQL:// URLs for testing
        MysqlRepository::new("mysql://root:password@localhost/test_ruci")
            .await
            .expect("Failed to connect to MySQL test database")
    }

    // Tests would be similar to sqlite.rs but require a MySQL instance
    // Including basic structure for reference
